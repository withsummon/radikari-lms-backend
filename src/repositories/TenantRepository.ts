...
export async function getAllByUserId(filters: EzFilter.FilteringQuery, user: UserJWTDAO) {
    const queryBuilder = new EzFilter.BuildQueryFilter()
    const usedFilters = queryBuilder.build(filters)
    ...
    return {
        entries: tenant.map((tenant: any) => ({
            ...
            name: tenant.name,
            description: tenant.description,
            tenantRole: user.role === Roles.ADMIN ? Roles.ADMIN : tenant.tenantUser[0].tenantRole,
        })),
        totalData,
        totalPage,
    }
