...
export class RabbitMQConnection {
	...
	async connect() {
		...
		while (retryCount < maxRetries) {
			try {
				Logger.info(
					`⌛️ Connecting to Rabbit-MQ Server (attempt ${retryCount + 1}/${maxRetries})`,
					{},
				)
				this.connection = await client.connect(process.env.AMQP_CONN_URL || "")

				Logger.info(`✅ Rabbit MQ Connection is ready`, {})

				this.channel = await this.connection.createChannel()
				Logger.info(`🛸 Created RabbitMQ Channel successfully`, {})
				Graceful.registerProcessForShutdown(
					`pubsub-connection-${ulid()}`,
					() => {
						this.channel.close()
					},
				)
				...
				if (retryCount >= maxRetries) {
					console.error(
						`Failed to connect after ${maxRetries} attempts. Giving up.`,
					)
	...
	async consume(queue: string, consumerHandler: HandlerCB) {
		Logger.info(
			`[CONSUMER] Attempting to declare queue: ${queue} with options: { durable: true }`,
			{},
		)
		...
		try {
			...
			if (
				...
				Logger.info(
					`[CONSUMER] Successfully recreated queue: ${queue} with correct configuration`,
					{},
				)
			} else {
				throw error
			...
			async (msg: any) => {
				{
					...
					while (retries < maxRetries) {
						try {
							...
							retries++
							Logger.error(
								`[CONSUMER] Error processing message (attempt ${retries}/${maxRetries}):`,
								error,
							)
							...
							if (retries === maxRetries) {
								...
								Logger.error(
									`[CONSUMER] Max retries reached, message acknowledged and removed from queue`,
									{
										error: error,
									},
								)
