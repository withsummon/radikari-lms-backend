...
export async function createRole(c: Context): Promise<TypedResponse> {
    ...
    return response_created(c, serviceResponse.data, "Successfully created new Tenant Role!")
}
...
export async function updateRoleAccess(c: Context): Promise<TypedResponse> {
    ...
    return response_success(c, serviceResponse.data, "Successfully updated Tenant Role Access!")
}

export async function getEnabledFeaturesByTenantRoleId(c: Context): Promise<TypedResponse> {
    const id = c.req.param("tenantRoleId")

    const serviceResponse = await AccessControlListService.getEnabledFeaturesByRoleId(id)
    if (!serviceResponse.status) {
        return handleServiceErrorWithResponse(c, serviceResponse)
    }
