...
export async function getByTenantId(c: Context): Promise<TypedResponse> {
    ...
    return response_success(c, serviceResponse.data, "Successfully fetched AiPrompt by id!")
}
...
export async function createOrUpdateByTenantId(c: Context): Promise<TypedResponse> {
    ...
    const serviceResponse = await AiPromptService.upsertByTenantId(tenantId, data, user.id)

    if (!serviceResponse.status) {
        return handleServiceErrorWithResponse(c, serviceResponse)
    }

    return response_success(c, serviceResponse.data, "Successfully updated AiPrompt!")
}
