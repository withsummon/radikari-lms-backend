...
export async function create(c: Context): Promise<TypedResponse> {
  ...
  const serviceResponse = await KnowledgeService.create(
    user.id,
    tenantId,
    data,
  );
...
export async function getAll(c: Context): Promise<TypedResponse> {
  ...
  const serviceResponse = await KnowledgeService.getAll(
    user,
    tenantId,
    filters,
  );
  ...
  const dataWithOverdue = attachOverdueToKnowledgeList(
    serviceResponse.data,
    24,
  );
