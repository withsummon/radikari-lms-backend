...
export async function update(c: Context): Promise<TypedResponse> {
    ...
    const serviceResponse = await KnowledgeService.update(id, tenantId, data, user.id)
...
export async function deleteById(c: Context): Promise<TypedResponse> {
    ...
    const tenantId = c.req.param("tenantId")
    const user: UserJWTDAO = c.get("jwtPayload")
    const serviceResponse = await KnowledgeService.deleteById(id, tenantId, user.id)
