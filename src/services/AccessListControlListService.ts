...
export async function updateRoleAccess(
	...
	try {
		...
		for (const enabledFeature of enabledFeatureKeys) {
			...
			const actionExists =
				await AccessControlListRepository.findActionByFeatureAndName(
					featureName,
					actionName,
				)
			...
			if (actionExists) {
				...
				const existingMapping =
					await AccessControlListRepository.findAccessMapping(
						tenantRoleId,
						featureName,
						actionName,
					)
		...
		await AccessControlListRepository.updateRoleAccess(
			createMappings,
			deleteMappings,
		)
		...
		Logger.error(`AccessListControlListService.updateRoleAccess `, {
			error: err,
		})
		return HandleServiceResponseCustomError("Internal Server Error", 500)
	}
