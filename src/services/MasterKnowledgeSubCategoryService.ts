...
export async function create(data: MasterKnowledgeSubCategoryDTO):Promise<ServiceResponse<MasterKnowledgeSubCategory | {}>> {
    try {
...
export async function getAll(filters: EzFilter.FilteringQuery): Promise<ServiceResponse<EzFilter.PaginatedResult<MasterKnowledgeSubCategory[]>|{}>> {
    try {
...
export async function getById(id: string): Promise<ServiceResponse<MasterKnowledgeSubCategory | {}>> {
    try {
        let masterKnowledgeSubCategory = await MasterKnowledgeSubCategoryRepository.getById(id)
        
        if (!masterKnowledgeSubCategory) return HandleServiceResponseCustomError("Invalid ID", ResponseStatus.NOT_FOUND)
...
export async function update(id: string, data: MasterKnowledgeSubCategoryDTO): Promise<ServiceResponse<UpdateResponse>> {
    try {
        let masterKnowledgeSubCategory = await MasterKnowledgeSubCategoryRepository.getById(id);

        if (!masterKnowledgeSubCategory) return HandleServiceResponseCustomError("Invalid ID", ResponseStatus.NOT_FOUND)

        masterKnowledgeSubCategory = await MasterKnowledgeSubCategoryRepository.update(id, data)
