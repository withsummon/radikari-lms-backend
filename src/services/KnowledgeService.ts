...
export async function approveById(
    ...
    try {
        ...
        await KnowledgeRepository.updateStatus(id, userId, status, data.action, data.comment)
...
export async function archiveOrUnarchiveKnowledge(id: string, userId: string, tenantId: string) {
    try {
        const knowledge = await KnowledgeRepository.getById(id)
