...
export async function create(data: MasterKnowledgeCaseDTO):Promise<ServiceResponse<MasterKnowledgeCase | {}>> {
    try {
...
export async function getAll(filters: EzFilter.FilteringQuery): Promise<ServiceResponse<EzFilter.PaginatedResult<MasterKnowledgeCase[]>|{}>> {
    try {
...
export async function getById(id: string): Promise<ServiceResponse<MasterKnowledgeCase | {}>> {
    try {
        let masterKnowledgeCase = await MasterKnowledgeCaseRepository.getById(id)
        
        if (!masterKnowledgeCase) return HandleServiceResponseCustomError("Invalid ID", ResponseStatus.NOT_FOUND)
...
export async function update(id: string, data: MasterKnowledgeCaseDTO): Promise<ServiceResponse<UpdateResponse>> {
    try {
        let masterKnowledgeCase = await MasterKnowledgeCaseRepository.getById(id);
