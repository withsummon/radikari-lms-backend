...
import { CreateManyNotificationDTO, CreateNotificationDTO } from "$entities/Notification"
...
export async function createMany(data: CreateManyNotificationDTO): Promise<ServiceResponse<{}>> {
    try {
...
export async function notifyTenantUsers(
    ...
    try {
        ...
        Logger.info(`NotificationService.notifyTenantUsers: Sent ${userIds.length} notifications`, {
            tenantId,
            type,
            referenceId,
        })
...
export async function notifyTenantRoleUsers(
    ...
    try {
        const tenantRoleUsers = await TenantUserRepository.getByTenantRoleId(tenantRoleId)

        if (tenantRoleUsers.length === 0) {
            return HandleServiceResponseCustomError("No users found", ResponseStatus.NOT_FOUND)
        }
...
export async function markAsRead(id: string, userId: string): Promise<ServiceResponse<{}>> {
    try {
...
export async function markAllAsRead(userId: string): Promise<ServiceResponse<{}>> {
    try {
