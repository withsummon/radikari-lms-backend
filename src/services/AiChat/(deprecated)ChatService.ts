...
export async function getChatRoomHistory(
	...
	ServiceResponse<EzFilter.PaginatedResult<AiChatRoomMessage[]> | {}>
> {
	try {
		const aiChatRoom = await AiChatRoomRepository.getByIdAndUserId(
			chatRoomId,
			userId,
		)
...
export async function chat(
	data: AiChatRoomMessageCreateDTO,
	aiChatRoomId: string,
	userId: string,
) {
	try {
...
export async function streamMessage(
	...
	try {
		...
		const chatHistory =
			await AiChatRoomMessageRepository.getLatestChatRoomHistory(chatRoomId)
		console.log("Chat History:", chatHistory)

		const requestBody = await AiChatRoomRepository.buildRequestBody(
			payload,
			chatHistory,
			user,
		)
...
async function processStreamEvent(
	...
	newSources: AiClientSourceToSave[]
}> {
	try {
		...
		switch (parsedData.type) {
			...
			case "sources":
				...
				const newSources: AiClientSourceToSave[] = sourceResponse.sources.map(
					(source) => ({
				...
				for (const source of sources) {
					...
					if (knowledgeExists) {
						...
						const existingRelation =
							await tx.aiChatRoomMessageKnowledge.findFirst({
								where: {
...
export async function archiveOrUnarchiveAiChatRoom(
	...
	try {
		...
		await AiChatRoomRepository.archiveOrUnarchiveAiChatRoom(
			id,
			userId,
			aiChatRoom.isArchived,
		)
		return HandleServiceResponseSuccess({})
	} catch (error) {
