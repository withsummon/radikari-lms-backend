...
export async function validateMasterKnowledgeCaseSchema(c: Context, next: Next) {
    ...
    if (caseExist) {
        invalidFields.push(Helpers.generateErrorStructure("name", "name already used"))
    }
...
export async function validateUpdateMasterKnowledgeCaseSchema(c: Context, next: Next) {
    const data: MasterKnowledgeCaseDTO = await c.req.json()
