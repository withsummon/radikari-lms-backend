...
export async function validateTenantUserUpdateSchema(c: Context, next: Next) {
	...
	if (data.length === 0) {
		invalidFields.push(
			Helpers.generateErrorStructure("data", "Data is required"),
		)
	}

	for (const tenantUser of data) {
		invalidFields.push(
			...Helpers.validateSchema(TenantUserUpdateSchema, tenantUser),
		)
	}
...
export async function validateTenantUserCreateSchema(c: Context, next: Next) {
	...
	if (!tenantRole) {
		invalidFields.push(
			Helpers.generateErrorStructure("tenantRoleId", "tenantRole not found"),
		)
	}

	if (!user) {
		invalidFields.push(
			Helpers.generateErrorStructure("userId", "user not found"),
		)
	}
