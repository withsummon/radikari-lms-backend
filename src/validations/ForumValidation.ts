...
export async function validateForumSchema(c: Context, next: Next) {
    ...
    let invalidFields: Helpers.ErrorStructure[] = Helpers.validateSchema(ForumSchema, data)
...
export async function validateForumCommentSchema(c: Context, next: Next) {
    ...
    let invalidFields: Helpers.ErrorStructure[] = Helpers.validateSchema(ForumCommentSchema, data)
    ...
    if (data.replyToCommentId) {
        ...
        if (!replyToComment) {
            ...
            if (replyToComment.replyToCommentId) {
                invalidFields.push({
                    field: "replyToCommentId",
                    message: "Reply to comment is not allowed to reply to another comment",
                })
            }
        }
    }
