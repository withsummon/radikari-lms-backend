...
export async function seedMasterKnowledgeCategorySubCategoryAndCase(prisma: PrismaClient) {
    const knowledges = await prisma.knowledge.findMany()
    const cases = knowledges.map((knowledge) => knowledge.case).filter((c) => c !== null)
    const categories = knowledges.map((knowledge) => knowledge.category).filter((c) => c !== null)
